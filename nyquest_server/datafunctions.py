...
class DataFunctions:
 ...
 def requesthandler(self,buff,sessionid,ghashclient,gwritearray):
   global sqlconnection
   logmod.logmsg("reqid="+str(buff[3]),"inside requesthandler sesionid="+str(sessionid),'debug')
   ...
   sesid=sessionid
   logmod.logmsg(devid,"inside Auth request ","debug")
   ...
   resetmode='false'
   otamode='false'
   for a in gwritearray:
      if (a==devid):        
         if(servertype=='LV'):
            otacnt=mydb.getOTACount(devid)
            if (otacnt>0):
               otamode='true'
               logmod.logmsg(devid," otamode="+otamode,'debug')
            writecnt,statecnt,resetcnt=mydb.getWriteCount(devid)
            if (resetcnt>0):
                resetmode='true'
                logmod.logmsg(devid," resetmode="+resetmode,'debug')
       
            if(resetmode=='false'):
                #sqlconnection,statecnt=mydb.getStatechangeCount(devid,sqlconnection)
                if (statecnt>0):
                   statemode='true'
                   logmod.logmsg(devid, " statemode="+statemode,'debug')
            if(resetmode=='false' and statemode=='false'):
                 #sqlconnection,writecnt=mydb.getWriteCount(devid,sqlconnection)
                 if (writecnt>0):
                    writemode='true'
                    logmod.logmsg(devid," writemode="+writemode,'debug')   
   
   
   for x in ghashclient['client']:    
     if (x['devid']==devid and x['readmode']=='true') :
         logmod.logmsg(devid,"configuraion read mode ","debug")
         r1=9
         r2=0
         #print("readmode for devid ",devid)
         
   if (writemode=='true'):
         logmod.logmsg(devid,"configuraion write mode ","debug")
         r1=9
         r2=0
   if (statemode=='true') :
         logmod.logmsg(devid,"configuraion statemode devid","debug")
         r1=9
         r2=0
   if (resetmode=='true') :
         logmod.logmsg(devid,"configuraion statemode","debug")
         r1=9
         r2=0     
   if (otamode=='true') :
         logmod.logmsg(devid,"configuraion otamode","debug")
         r1=9
         r2=0
 ...
 def readdatahandler(self,buff,deviceid):
   ...
   logmod.logmsg(deviceid,"inside readdatahandler",'debug')
 ...
 def otahandler(self,devid,sesionid,dataindex,fwstartaddress,filename,buff):
    #global sqlconnection
    logmod.logmsg(devid,"inside otahandler dataindex="+str(dataindex)+" fwstartaddress="+str(fwstartaddress),'debug')
         ...
         while rawcode:   
            if(idx>0 and  dataindex==idx):            
              ...
              if(self.checkprevdata(rawcode,fwstartaddress,sesionid,buff)==True):
                ...
                if(len(rawcode)<1024): 
                  dataindex=-1
                  logmod.logmsg(devid,"inside otahandler read completed devid" ,'debug')
              else:
                sentmsg=self.processotadata(rawcode,fwstartaddress,sesionid)
 ...
 def writehandler(self,devid,sesionid):
   ...
   logmod.logmsg(devid,"inside writehandler data="+str(data)+" addloc="+addloc,'debug')
   s1= sesionid & 0xFF
   s2 = sesionid>>8 & 0xFF
   s3 = sesionid>>16 & 0xFF
   s4 = sesionid >>24 & 0xFF
 ...
 def statechangehandler(self,devid,sesionid):
   ...
   logmod.logmsg(devid,"inside statechangehandler asserttype="+asserttype,'debug')
 ...
 def resethandler(self,devid,sesionid):
   global sqlconnection
   logmod.logmsg(devid,"inside resethandler",'debug')   
   readres = (c_uint8 * 15)(*range(15))
 ...
 def commonflaghandler(self,devid,sesionid,asserttype):
   ...
   logmod.logmsg(devid,"inside commonflaghandler",'debug')
   readres = (c_uint8 * 15)(*range(15))
 ...
 def logrequesthandler(self,buff,ghashclient):
  #logmod.logmsg("--","inside logrequesthandle",'debug')
  global ghashdevice
  ...
  solardisconnection='false'
  inverterfail_messagesent='false'
  sol_messagesent='false'
  sol_dis_cnt=0;
  #dt_string = now.strftime("%d%m%Y%H%M%S")
  dt_string = now.strftime("%Y-%m-%d %H:%M:%S")
  time_stamp = (c_uint8 * 7)(*range(7))
  ...
  for x in ghashclient['client']:      
      if x['sessionid']==sessionid:                 
          devid=x['devid']
  logmod.logmsg(devid,"inside logrequesthandle sessionid="+str(sessionid),'debug')        
  if(devid!=''):
   strbuff=""
   for a in buff:
     strbuff=strbuff+","+str(a)    
   ...
   time_stamp[5]=(c_uint8)(int(now.strftime("%S"))) 
   time_stamp[6]=(c_uint8)(int(now.strftime("%f"))) 
   ...
   if(servertype=='LV'):
     if (logpayload.dev_status_flag&(1<<13)):
       readmode='true'
       logmod.logmsg(devid," readmode="+readmode,'debug')
   
  
  
       readmode='true'
   if (logpayload.dev_status_flag&(1<<12)):
       inverterfailure='true'
       logmod.logmsg(devid,"  inverterfailure="+inverterfailure,'debug')
   if (logpayload.algorithm_status_flag&(1<<10)=='true' and int(time_stamp[3]) > 7 and int(time_stamp[3]) < 17 and int(logpayload.log_type)==1):
       solardisconnection='true'
       logmod.logmsg(devid,"  solardisconnection="+solardisconnection,'debug')
   ...
   logtime=str(time_stamp[2]+2000)+'-'+str(time_stamp[1]).zfill(2)+'-'+str(time_stamp[0]).zfill(2)+' '+str(time_stamp[3]).zfill(2)+':'+str(time_stamp[4]).zfill(2)+':'+str(time_stamp[5]).zfill(2)+'.'+str(time_stamp[6]).zfill(3)
   logmod.logmsg(devid,"inside logrequesthandler logdata logtime="+logtime,'debug')   
   stat=0
   ...
   for x in ghashdevice['device']:
      if x['devid']==devid: 
          lastpayload=x['lastpayload']
          
   
   if(lastpayload!=strpl):
    narraycnt=0
    prevsolardis='false'
    previnverterfail='false'
    previnvsent='false'
    prevsolsent='false'
    ...
    for x in ghashdevice['device']:
      if x['devid']==devid: 
          ...
          x['lastpayload']=strpl
          prevsolardis=x['solardisconnection']
          previnverterfail=x['inverterfailure']
          prevsoldiscnt=x['sol_dis_cnt']
          previnvsent=x['inverterfail_messagesent']
          prevsolsent=x['sol_messagesent']
          
          if(previnverterfail =='true' and inverterfailure=='false'):
            x['inverterfail_messagesent']='false'         
          x['inverterfailure']=inverterfailure
          x['solardisconnection']=solardisconnection
          if(solardisconnection=='true'):
            sol_dis_cnt=prevsoldiscnt+1
          else:
            sol_dis_cnt=0
            sol_messagesent='false'
                        
          x['sol_dis_cnt']=sol_dis_cnt
          if(sol_dis_cnt==10):
             sol_messagesent='true'
             mydb.insertAlertlog(1,devid,logtime)
          else:
             sol_messagesent='false'
          x['sol_messagesent']=sol_messagesent
          narraycnt=1
    if(narraycnt==0):
      d={}
      d['devid']=devid              
      d['lastpayload']=strpl
      d['inverterfailure']=inverterfailure
      d['solardisconnection']=solardisconnection
      d['sol_dis_cnt']=sol_dis_cnt
      d['sol_messagesent']='false'
      d['inverterfail_messagesent']='false'
      ghashdevice['device'].append(d)
      
    if(previnvsent=='false' and inverterfailure=='true'):
       for x in ghashdevice['device']:
         if x['devid']==devid: 
            x['inverterfail_messagesent']='true'
       mydb.insertAlertlog(2,devid,logtime)
    #try:
    arrd={}              
    #arrd['jsondata']=json.loads(jsondata)
    #arrd['jsondata']=jsondata
    #ghashlogdata['logdata'].append(arrd)
    gmongo_docs.append(json.loads(jsondata))
    logmod.logmsg(devid,"inside insertSummaryLog gmongo_docs length="+str(len(gmongo_docs)), 'error')
    if(len(gmongo_docs)>=20 or logpayload.log_type=='1' or logpayload.log_type==1):
        mongoconnection=mdb.insertlog(gmongo_docs,mongoconnection)
        gmongo_docs=[]
    #except:
    ...
    duplicaterecord=1
    logmod.logmsg(devid,"Log Duplicate record " ,'debug')  
  ...
  return readmode,logrestag,duplicaterecord
 def configExitOTA(self,devid,sesionid):
   logmod = logModule()
   servfn=ServerFunctions()
   logmod.logmsg(devid,"inside configExitOTA ",'debug')
 ...
 def configExit(self,devid,sesionid):
   logmod = logModule()
   servfn=ServerFunctions()
   logmod.logmsg(devid,"inside configExit",'debug')
