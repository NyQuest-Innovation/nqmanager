...
class mysqldb:
 ...
 def getDeviceForWrite(self):
   ...
   try:
      ...
      if connection.is_connected():       
        cursor = connection.cursor(buffered=True)
        cursor.execute("select dev_id from tbl_icon_device where id in (select device_id from tbl_icon_threshold_log where b_updated=0) or id in (select device_id from tbl_icon_statechange where b_updated=0) or id in (select device_id from tbl_icon_device_reset where b_reset=0)")
